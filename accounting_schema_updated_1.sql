...
CREATE TRIGGER update_general_ledger_timestamp
...
RETURNS TRIGGER AS $$
BEGIN
    IF EXISTS (SELECT 1 FROM fiscal_period WHERE period_id = NEW.period_id AND is_closed = true) THEN
        RAISE EXCEPTION 'No se puede modificar el periodo % porque está cerrado', NEW.period_id;
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
CREATE TRIGGER general_ledger_closed_check
...
CREATE INDEX idx_general_ledger_entity_id ON general_ledger(entity_id);
CREATE INDEX idx_general_ledger_account_id ON general_ledger(account_id);
CREATE INDEX idx_general_ledger_period_id ON general_ledger(period_id);
CREATE INDEX idx_general_ledger_entry_id ON general_ledger(entry_id);
...
BEGIN
    ...
    -- Generar líneas en general_ledger
    INSERT INTO general_ledger (
        entry_id, account_id, debit, credit, currency_code, cost_center_id, project_id, partner_id, entry_date, entity_id, period_id
    )
    SELECT
        v_entry_id,
    ...
    -- Verificar balance
    IF (SELECT ABS(SUM(debit) - SUM(credit)) FROM general_ledger WHERE entry_id = v_entry_id) > 0.01 THEN
